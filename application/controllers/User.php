...
class User extends CI_Controller {
   ...
   function sendemail($email,$saltid){  
    ...
    public function check_login()
    {
        ...
        else
        {
            ...
            if(!empty($res))
            {
                if($res[0]['status'] == '1')
                {
                    $data['user'] = $res[0]['first_name'];
                    $this->setSession($res[0]['id_user'],$res[0]['first_name'],$res[0]['id_jenis_user'],$res[0]['email']);
    ...
    function setSession($userId,$userName,$userLevel,$email) {
